...
export default function EquipeUpdateForm(props) {
  const {
    id: idProp,
    equipe: equipeModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = equipeRecord
      ? { ...initialValues, ...equipeRecord }
      : initialValues;
    setNome(cleanValues.Nome);
    setFuncao(cleanValues.Funcao);
    setAtivo(cleanValues.Ativo);
    setErrors({});
  };
  const [equipeRecord, setEquipeRecord] = React.useState(equipeModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Equipe, idProp)
        : equipeModelProp;
      setEquipeRecord(record);
    };
    queryData();
  }, [idProp, equipeModelProp]);
  React.useEffect(resetStateValues, [equipeRecord]);
  const validations = {
    Nome: [{ type: "Required" }],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Equipe.copyOf(equipeRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "EquipeUpdateForm")}
      {...rest}
    >
      <TextField
        label="Nome"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || equipeModelProp)}
