...
export default function SondaUpdateForm(props) {
  const {
    id: idProp,
    sonda: sondaModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = sondaRecord
      ? { ...initialValues, ...sondaRecord }
      : initialValues;
    setNomeSonda(cleanValues.NomeSonda);
    setAtivo(cleanValues.Ativo);
    setErrors({});
  };
  const [sondaRecord, setSondaRecord] = React.useState(sondaModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Sonda, idProp)
        : sondaModelProp;
      setSondaRecord(record);
    };
    queryData();
  }, [idProp, sondaModelProp]);
  React.useEffect(resetStateValues, [sondaRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Sonda.copyOf(sondaRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "SondaUpdateForm")}
      {...rest}
    >
      <TextField
        label="Nome sonda"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || sondaModelProp)}
