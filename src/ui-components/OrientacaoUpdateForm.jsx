...
export default function OrientacaoUpdateForm(props) {
  const {
    id: idProp,
    orientacao: orientacaoModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = orientacaoRecord
      ? { ...initialValues, ...orientacaoRecord }
      : initialValues;
    setCodigo(cleanValues.Codigo);
    setSigla(cleanValues.Sigla);
    setDescricao(cleanValues.Descricao);
    setAtivo(cleanValues.Ativo);
    setErrors({});
  };
  const [orientacaoRecord, setOrientacaoRecord] =
    React.useState(orientacaoModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Orientacao, idProp)
        : orientacaoModelProp;
      setOrientacaoRecord(record);
    };
    queryData();
  }, [idProp, orientacaoModelProp]);
  React.useEffect(resetStateValues, [orientacaoRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Orientacao.copyOf(orientacaoRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "OrientacaoUpdateForm")}
      {...rest}
    >
      <TextField
        label="Codigo"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || orientacaoModelProp)}
