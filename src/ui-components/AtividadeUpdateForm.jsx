...
export default function AtividadeUpdateForm(props) {
  const {
    id: idProp,
    atividade: atividadeModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = atividadeRecord
      ? { ...initialValues, ...atividadeRecord }
      : initialValues;
    setNomeAtividade(cleanValues.NomeAtividade);
    setCodigo(cleanValues.Codigo);
    setTipo(cleanValues.Tipo);
    setAtivo(cleanValues.Ativo);
    setInfoPerfuracao(cleanValues.InfoPerfuracao);
    setErrors({});
  };
  const [atividadeRecord, setAtividadeRecord] =
    React.useState(atividadeModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Atividade, idProp)
        : atividadeModelProp;
      setAtividadeRecord(record);
    };
    queryData();
  }, [idProp, atividadeModelProp]);
  React.useEffect(resetStateValues, [atividadeRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Atividade.copyOf(atividadeRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "AtividadeUpdateForm")}
      {...rest}
    >
      <TextField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || atividadeModelProp)}
